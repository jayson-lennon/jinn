//! Todo list model - phased task tracking for agent sessions.
//!
//! Provides a structured task list with one level of nesting: phases contain tasks.
//! The data model is stored per-session on the kernel's `SessionCore` and
//! persists across restarts via the existing session serialization pipeline.
...
impl PhaseId {
    ...
    /// Test-only constructor: derives an ID from a fixed string.
    #[doc(hidden)]
