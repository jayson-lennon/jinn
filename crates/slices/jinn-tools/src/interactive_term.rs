...
pub fn execute(call: ToolCall, ctx: ToolContext) -> BoxedToolFuture {
    ...
    let max_wait = crate::orchestrator::extract_max_duration(&call.arguments)
    ...
    F: std::future::Future<Output = T>,
{
    let pacer = bus.zip(stream_ctx).map(|(bus, ctx)| {
        tokio::spawn(async move {
            ...
            loop {
                tick.tick().await;
                bus.publish(jinn_tools_msg::ToolExecutionOutput {
                    session_id: ctx.session_id.clone(),
                    tool_call_id: ctx.tool_call_id.clone(),
                    output: "…".to_owned(),
                    kind: jinn_tools_msg::ToolOutputKind::Normal,
                })
                .await;
            }
        })
...
mod tests {
    ...
    async fn keepalive_publishes_heartbeats_during_long_ask() {
        ...
        let beats = audit.of_type::<jinn_tools_msg::ToolExecutionOutput>();
        assert!(
            beats.len() >= 2,
    ...
    async fn keepalive_without_session_publishes_nothing() {
        ...
        assert!(
            audit
                .of_type::<jinn_tools_msg::ToolExecutionOutput>()
                .is_empty()
        );
    }
}
