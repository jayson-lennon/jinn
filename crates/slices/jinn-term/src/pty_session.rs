...
fn new_screen_handle(rows: u16, cols: u16) -> ScreenHandle {
    ...
    let shared = SharedTerminal::new(Emulator::new(rows, cols, TRANSCRIPT_LINES), version_tx);
...
mod tests {
    ...
    async fn wiring() -> ScreenWiring {
        ...
        ScreenWiring {
            bus: harness.bus(),
            state,
            chat: jinn_core_types::SessionId::new(),
        }
    }
    ...
    async fn spawned_child_echoes_input_back_through_screen() {
        ...
        let (mut session, pump) =
            PtySession::spawn("cat", Path::new("/tmp"), default_size(), wiring().await)
                .expect("spawn cat");
    ...
    async fn try_wait_reports_exit_code_after_natural_exit() {
        ...
        let (session, pump) =
            PtySession::spawn("exit 7", Path::new("/tmp"), default_size(), wiring().await)
                .expect("spawn exit");
    ...
    async fn self_exited_program_is_reaped_not_zombied() {
        ...
        // zombie).
        let (session, pump) = PtySession::spawn(
            "exec true",
            Path::new("/tmp"),
            default_size(),
            wiring().await,
