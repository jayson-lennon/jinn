//! Sliding window input popup rendering — a centered overlay for adjusting the window size.
...
/// Computes the popup rectangle for the sliding window input overlay.
pub fn sliding_window_popup_rect(area: Rect) -> Rect {
...
/// Renders the sliding window input popup.
///
/// Shows a centered popup with:
/// - Title: " Sliding Window Size "
/// - Input line with cursor showing the current value
pub fn render_sliding_window_input(frame: &mut Frame<'_>, area: Rect, state: &AppState) {
    let input_state = &state.frontend.sliding_window_input;
    let theme = &state.frontend.theme;

    let popup_area = sliding_window_popup_rect(area);
...
mod tests {
    ...
    #[rstest::rstest]
    fn sliding_window_popup_shows_title() {
        // Given a state in SlidingWindowInput scope with input.
        ...
        terminal
            .draw(|frame| {
                render_sliding_window_input(frame, area, &state);
            })
            .unwrap();
        ...
        let title_text = " Sliding Window Size ";
    ...
    #[rstest::rstest]
    fn sliding_window_popup_shows_input_text() {
        // Given a state with input "20".
        let mut state = AppState::default();
        state
            .frontend
            .scope_stack
        ...
        let row_text: String = (inner_x..inner_x + 20)
            ...
            .collect();
        assert!(
            row_text.starts_with("> 20"),
            "expected '> 20' on input line, got: {row_text}"
        );
    }

    #[rstest::rstest]
    fn sliding_window_popup_clears_background() {
    ...
    #[rstest::rstest]
    fn sliding_window_popup_rect_is_centered() {
        // Given an 80x24 area.
        let area = Rect::new(0, 0, 80, 24);

        // When computing popup rect.
        let popup = sliding_window_popup_rect(area);
