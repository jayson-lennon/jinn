...
fn resolve_session(
    ...
    current: Option<&crate::protocol::SessionId>,
) -> Result<crate::protocol::SessionId, String> {
    if let Some(id) = explicit {
        return Ok(crate::protocol::SessionId::from(id));
    }
    current
        .cloned()
...
pub fn execute(call: ToolCall, ctx: ToolContext) -> BoxedToolFuture {
    ...
    Box::pin(async move {
        ...
        let window = match &args.1 {
            FetchMode::Anchored { entry_id, context } => {
                let anchor = crate::protocol::ChatEntryId::from(entry_id.clone());
